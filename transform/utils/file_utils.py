...
def export_df_to_binary(df, output_file):
    # Convert DataFrame to records for export_to_binary function
    records = []
    for index, row in df.iterrows():
        mz_values = row['mz']
        intensity_values = row['int']
        for mz, intensity in zip(mz_values, intensity_values):
            records.append((index, 0.0, mz, intensity, 1))

    export_to_binary(records, output_file)
...
def import_from_binary(file_path):
    ...
    mz_values = [np.array(mz, dtype=np.float64) for mz in mz_values]
    intensity_values = [np.array(intensity, dtype=np.float64) for intensity in intensity_values]
